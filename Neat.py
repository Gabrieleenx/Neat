...
def check_extinction(species, species_fitness, species_gen_stalled, gen_to_extinct, old_species):
    ...
    for specie_ in range(len(species)):
        ...
        if not species[specie_]:
            extinction.append(specie_)
    for del_ in sorted(extinction, reverse=True):
        del old_species[del_]
        del species[del_]
        del species_gen_stalled[del_]
        del species_fitness[del_]
    return species, species_fitness, species_gen_stalled, old_species
...
class Neat(object):
    ...
    def train(self, p_survivors, weight_mutations, enable_ratio):
        ...
        # print('---after specie---')
        self.species, self.species_fitness, self.species_gen_stalled, self.old_species = \
        ...
        for specie_index in range(len(offspring)):
            ...
            if offspring_cnt == 0 or not offspring_cnt:  # might have fixed a bug ??
            ...
            try:
                ...
                print('---------------error-----------------')
                print('self.species', self.species[specie_index])
                print('offspring_cnt', offspring_cnt)
